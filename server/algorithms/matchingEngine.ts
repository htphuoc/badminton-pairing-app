...
function bestTeamSplit(four: EnrichedPlayer[]): { team1: string[]; team2: string[]; skillDiff: number } {
  ...
  for (const [a, b] of splits) {
    ...
    if (diff < best.skillDiff) {
      best = {
        team1: a.map(i => four[i].memberId),
        team2: b.map(i => four[i].memberId),
        skillDiff: diff,
      };
    }
  }
...
function mixedTeamSplit(four: EnrichedPlayer[]): { team1: string[]; team2: string[]; skillDiff: number } | null {
  ...
  for (const opt of options) {
    ...
    if (diff < best.skillDiff) {
      best = {
        team1: opt.t1.map(p => p.memberId),
        team2: opt.t2.map(p => p.memberId),
        skillDiff: diff,
      };
    }
  }
...
function buildCandidateFours(sorted: EnrichedPlayer[]): { four: EnrichedPlayer[]; type: MatchType; note: string }[] {
  ...
  const push = (four: EnrichedPlayer[], type: MatchType, note: string) => {
    ...
    const key = playerSetKey(four.map(p => p.memberId));
...
export const generateMatchSuggestion = (params: {
  ...
  const sorted = [...waitingPlayers].sort((a, b) => {
    const sa = priorityScores.get(a.memberId) ?? 0;
    const sb = priorityScores.get(b.memberId) ?? 0;
    return sb - sa;
  });
  ...
  const pick = candidates.find(c => !isExcludedSet(c.four.map(p => p.memberId), excludePlayerSets));
  if (!pick) return null;
  ...
  return {
    players: selectedFour.map(p => p.memberId),
    team1: split.team1,
    team2: split.team2,
    type: matchType,
    score,
